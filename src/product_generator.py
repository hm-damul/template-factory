    ...
    (function() {{
      ...
      function isLocalPreview() {{
        try {{
          ...
          // 8090/8088(preview), 8099(dashboard) 모두 로컬로 간주
...
if __name__ == "__main__":
    ...
    try:
        ...
        # 제품 자산 생성 실행
        assets = generator.generate_product_assets(test_config)
        
        # 파일 저장
        write_text(output_dir / "index.html", assets["landing_html"])
        write_text(output_dir / "checkout.html", assets["checkout_html"])
        
        logger.info(f"Generated index.html and checkout.html in {output_dir}")
