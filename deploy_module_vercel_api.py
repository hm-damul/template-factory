...
def _get_headers() -> Dict[str, str]:
    token = os.getenv("VERCEL_TOKEN", "").strip() or os.getenv("VERCEL_API_TOKEN", "").strip()
    if not token:
        # Try to load from Config if available
        try:
            from src.config import Config
            token = Config.VERCEL_API_TOKEN
        except ImportError:
            pass
            
    if not token:
        raise RuntimeError("VERCEL_TOKEN or VERCEL_API_TOKEN is missing in environment.")
    return {
