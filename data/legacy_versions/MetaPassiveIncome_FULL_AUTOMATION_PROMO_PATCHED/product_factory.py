...
class ProductConfig:
    ...
    price_usd: float = 29.0  # 기본 가격(표시용)
...
def write_assets(assets_dir: Path, product_id: str, topic: str) -> None:
    """assets 폴더에 최소 아이콘(SVG) 등을 만든다."""
...
def generate_one(cfg: ProductConfig) -> Dict[str, object]:
    ...
    if cfg.include_assets:
        write_assets(product_dir / "assets", cfg.product_id, cfg.topic)
