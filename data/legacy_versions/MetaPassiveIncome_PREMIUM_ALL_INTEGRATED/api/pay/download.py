...
- GET     /api/pay/download?token=... (preferred) or ?order_id=...
...
from payment_api import download_for_order, download_for_token
...
def handler(request):
    ...
    token = _get_query_param(request, "token").strip()
    order_id = _get_query_param(request, "order_id").strip()
    if not token and not order_id:
        return _resp(400, {"error": "token_or_order_id_required"}, "GET, OPTIONS")

    project_root = Path(__file__).resolve().parents[2]
    info = (
        download_for_token(project_root=project_root, token=token)
        if token
        else download_for_order(project_root=project_root, order_id=order_id)
    )
    if not info.get("ok"):
